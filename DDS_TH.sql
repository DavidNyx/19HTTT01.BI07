--CREATE DATABASE DDS_TH
--GO
--USE DDS_TH
--GO
CREATE TABLE NAM
(
	ID_NAM INT IDENTITY(1,1),
	NAM INT,
	CONSTRAINT PK_N PRIMARY KEY(ID_NAM)
)
GO

CREATE TABLE THANG_TRONG_NAM
(
	ID_TTN INT IDENTITY(1,1),
	THANG INT,
	QUY INT,
	ID_NAM INT,
	CONSTRAINT PK_TTN PRIMARY KEY(ID_TTN)
)
GO

CREATE TABLE NGAY_TRONG_THANG
(
	ID_NTT INT IDENTITY(1,1),
	NGAY INT,
	ID_TTN INT,
	CONSTRAINT PK_NTT PRIMARY KEY(ID_NTT)
)
GO

CREATE TABLE DIM_PHU
(
	ID INT IDENTITY(1,1),
	PHU_ID VARCHAR(255),
	PHU VARCHAR(255),
	PHU_GROUP VARCHAR(255),
	CITY VARCHAR(255),
	POSTALCODE VARCHAR(255),
	WEBSITE VARCHAR(255),
	LATITUDE VARCHAR(255),
	LONGITUDE VARCHAR(255),
	CONSTRAINT PK_P PRIMARY KEY(ID)
)
GO

CREATE TABLE DIM_VACCINES_BY_AGE
(
	VACCINES_BY_AGE_ID INT,
	ID INT IDENTITY(1,1),
	DATE DATE,
	PHU_ID VARCHAR(255),
	AGE_GROUP_ID INT,
	AT_LEAST_ONE_DOSE_CUMULATIVE INT,
	SECOND_DOSE_CUMULATIVE INT,
	FULLY_VACCINATED_CUMULATIVE INT,
	THIRD_DOSE_CUMULATIVE INT,
	CREATED_DATE DATETIME,
	UPDATED_DATE DATETIME,
	CONSTRAINT PK_VBA PRIMARY KEY(ID)
)
GO

CREATE TABLE DIM_ONGOING_OUTBREAK_PHU
(
	ID INT IDENTITY(1,1),
	ONGOING_OUTBREAK_PHU_ID INT,
	DATE DATE,
	PHU_NUM VARCHAR(255),
	OUTBREAK_GROUP VARCHAR(255),
	NUMBER_ONGOING_OUTBREAKS INT,
	CREATED_DATE DATETIME,
	UPDATED_DATE DATETIME,
	CONSTRAINT PK_OOP PRIMARY KEY(ID)
)
GO

CREATE TABLE DIM_AGE_GROUP
(
	ID INT IDENTITY(1,1),
	AGE_GROUP_ID INT,
	AGE_GROUP_TYPE VARCHAR(255),
	CREATED_DATE DATETIME,
	UPDATED_DATE DATETIME,
	CONSTRAINT PK_AG PRIMARY KEY(ID)
)
GO

CREATE TABLE DIM_CASE_REPORT
(
	ID INT IDENTITY(1,1),
	CASE_REPORT_ID VARCHAR(255),
	OUTCOME VARCHAR(255),
	GENDER VARCHAR(255),
	REPORTING_PHU_ID VARCHAR(255),
	SPECIMEN_DATE DATE,
	TEST_REPORTED_DATE DATE,
	CASE_ACQUISITION_INFO VARCHAR(255),
	ACCURATE_EPISODE_DT DATE,
	OUTBREAK_RELATED VARCHAR(255),
	AGE_ID INT,
	CREATED_DATE DATETIME,
	UPDATED_DATE DATETIME,
	CONSTRAINT PK_CR PRIMARY KEY(ID)
)
GO

CREATE TABLE DIM_COMPILED_CASE_DETAILS
(
	ID INT IDENTITY(1,1),
	DIM_COMPILED_CASE_DETAILS INT,
	OBJECT_ID VARCHAR(255),
	ROW_ID VARCHAR(255),
	DATE_REPORTED DATETIME,
	PHU_ID VARCHAR(255),
	AGE_GROUP_ID INT,
	GENDER VARCHAR(255),
	EXPOSURE VARCHAR(255),
	CASE_STATUS VARCHAR(255),
	PROVINCE VARCHAR(255),
	CREATED_DATE DATETIME,
	UPDATED_DATE DATETIME,
	CONSTRAINT PK_CCD PRIMARY KEY(ID)
)
GO

CREATE TABLE FACT_CASE
(
	ID_CASE INT IDENTITY(1,1),
	ID_NGAY INT,
	ID_PHU INT,
	ID_AGE_GROUP INT,
	ID_CASE_REPORT INT,
	ID_COMPILED_CASE_DETAILS INT,
	CONSTRAINT PK_FC PRIMARY KEY(ID_CASE)
)
GO

ALTER TABLE THANG_TRONG_NAM
ADD CONSTRAINT FK_TTN_N
FOREIGN KEY (ID_NAM)
REFERENCES NAM
GO

ALTER TABLE NGAY_TRONG_THANG
ADD CONSTRAINT FK_NTT_TTN
FOREIGN KEY (ID_TTN)
REFERENCES THANG_TRONG_NAM
GO

ALTER TABLE FACT_CASE
ADD CONSTRAINT FK_FC_N
FOREIGN KEY (ID_NGAY)
REFERENCES NGAY_TRONG_THANG
GO

ALTER TABLE FACT_CASE
ADD CONSTRAINT FK_FC_P
FOREIGN KEY (ID_PHU)
REFERENCES DIM_PHU
GO

ALTER TABLE FACT_CASE
ADD CONSTRAINT FK_FC_AG
FOREIGN KEY (ID_AGE_GROUP)
REFERENCES DIM_AGE_GROUP
GO

ALTER TABLE FACT_CASE
ADD CONSTRAINT FK_FC_CR
FOREIGN KEY (ID_CASE_REPORT)
REFERENCES DIM_CASE_REPORT
GO

ALTER TABLE FACT_CASE
ADD CONSTRAINT FK_FC_CCD
FOREIGN KEY (ID_COMPILED_CASE_DETAILS)
REFERENCES DIM_COMPILED_CASE_DETAILS
GO